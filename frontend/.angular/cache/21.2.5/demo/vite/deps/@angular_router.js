import {
  ...
  Title,
  ViewportScroller,
} from './chunk-UHMBFJIH.js';
import { LOCATION_INITIALIZED, PlatformLocation } from './chunk-6JYSAFDT.js';
import {
...
function isActive(url, router, matchOptions) {
  ...
  return computed(() =>
    containsTree(
      ...
      urlTree,
      __spreadValues(__spreadValues({}, subsetMatchOptions), matchOptions)
    )
  );
}
...
function runCanMatchGuards(injector, route, segments, urlSerializer, currentSnapshot, abortSignal) {
  ...
  const canMatchObservables = canMatch.map((injectionToken) => {
    ...
    const guardVal = isCanMatch(guard)
      ...
      : runInInjectionContext(injector, () => guard(route, segments, currentSnapshot));
  ...
  if (typeof ngDevMode === 'undefined' || ngDevMode) {
    ...
    if (route.redirectTo) {
      ...
      if (route.canMatch || route.canActivate) {
        throw new RuntimeError(
          4014,
          `Invalid configuration of route '${fullPath}': redirectTo and ${
            route.canMatch ? 'canMatch' : 'canActivate'
          } cannot be used together.Redirects happen before guards are executed.`
        );
      }
    }
...
function matchWithChecks(
  ...
  return runCanMatchGuards(
    ...
    currentSnapshot,
    abortSignal
  ).pipe(map((v) => (v === true ? result : __spreadValues({}, noMatch))));
}
...
This is currently a dev mode only error but will become a call stack size exceeded error in production in a future major version.`
  ...
  async matchSegmentAgainstRoute(injector, rawSegment, route, segments, outlet, parentRoute) {
    ...
    const createSnapshot = (result2) =>
      this.createSnapshot(
        injector,
        route,
        result2.consumedSegments,
        result2.parameters,
        parentRoute
      );
